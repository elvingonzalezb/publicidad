    ...
    <div class="page-body">
        <div class="panel panel-default">
                <div class="panel-heading">GALERIA DE PROYECTOS</div>
                                ...
                                <tr id="fila-<?=$value['id']?>">
                                    ...
                                    <td class="center">
                                        <form action="" id="estadoAjax">
                                            ...
                                            onclick="actualizarEstado($('#idAlb<?=$value['id']?>').val(),$('#estado<?=$value['id']?>').val(),'proyectos/ajaxEstado/1');return false;"><?=$status?></button>
                                            <span id="divResultado<?=$value['id']?>"></span>
                                        </form>
                                    </td>
                                    <td class="center">
                                        <a class="btn btn-info" href="dashboard/proyectos/editar_proyecto/<?=$value['id']?>">
                                            <i class="glyphicon glyphicon-edit icon-white"></i>
                                            Editar
                                        </a>
                                        <a class="btn btn-danger" href="javascript:eliminarItem(<?=$value['id']?>,'proyectos/delete_proyecto');">
                                            <i class="glyphicon glyphicon-trash icon-white"></i>
                                            Borrar
                                        </a>
                                        <?php 
                                            $href = 'dashboard/proyectos/lista_trabajo/'.$value['id'];
                                            $texto = 'Trabajos';
                                        ?>
