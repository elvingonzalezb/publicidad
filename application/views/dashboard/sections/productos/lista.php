		...
		<div class="panel-body">
			...
			<table class="table table-striped table-hover js-exportable dataTable" cellspacing="0" width="100%">
				...
				<tbody>
					...
					<?php $dstatus = (($value['destacado']==_ACTIVO) ? 'activo': 'inactivo')?>
					<?php $dclass = (($value['destacado']==_ACTIVO) ? 'btn-success': 'btn-warning')?>
					<tr id="fila-<?=$value['id']?>">
						<td class="center"><?=$key+1?></td>
						<td><img class="img-responsive" src="files/productos/thumbs/<?=$value['imagen']?>" alt="<?=$value['imagen_title']?>"></td>
						<td><?=$value['nombre']?></td>
						<td class="center">
							<form action="" id="destacadoAjax">
								<input type="hidden" id="destacado<?=$value['id']?>" value="<?=$value['destacado']?>">
								<button type="button" id="btnD-<?=$value['id']?>" class="btn <?=$dclass?>"
								onclick="actualizarDestacado(<?=$value['id']?>,$('#destacado<?=$value['id']?>').val());return false;"><?=$dstatus?></button>
								<span id="divResultadoD<?=$value['id']?>"></span>
							</form>
						</td>
						<td><?=$value['orden']?></td>
						<td class="center">
