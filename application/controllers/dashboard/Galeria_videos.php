...
class Galeria_videos extends CI_Controller {
	...
	public $names_video = array('link','nombre','NOTdescripcion','orden','destacado');
	...
	public function editar_album() {
		...
		if(!empty($_POST)){
			...
			if (validacionForm($this->names_album, 'controller') == FALSE){
				...
				$this->session->set_flashdata('message', '<strong>RESULTADO:</strong> Los datos se actualizaron correctamente.');
		...
		if(!empty($_POST)){
			...
			if (validacionForm($this->names_video, 'controller') == FALSE){
				...
				if($updImg){
						...
						$this->session->set_flashdata('message', '<strong>RESULTADO:</strong> Los datos se actualizaron correctamente.');					
				# Fin Subida
				header('Location: '.'../editar_video/'.$id_galeria_fotos);
			}
		}
