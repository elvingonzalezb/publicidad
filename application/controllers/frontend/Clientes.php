...
class Clientes extends CI_Controller {
	...
	public function __construct() {
        ...
        SessionCliente(array('login','logout','nuevo','activar_cuenta','olvido','restablecer','reenviar'));
    }
		...
		if (! empty($_POST)) {
			if (validacionForm($names, 'controller') == FALSE){
				...
				if ($this->Cliente->clienteLogin($correo, $password)) {
					...
					$this->session->set_flashdata('message', '<div class="form-group has-feedback"><div class="alert alert-danger">Correo o contraseña incorrecto.<br>Si su clave es correcta verificar que su cuenta este activa</div></div>');
				}
			}
    	}
		#captcha
		$seo = array('seo_title'=>'Ingresar :: '.dataConfig('nombre_comercial'),'seo_description'=>'Ingresa a tu cuenta.');
		$data['seo'] = $seo;
		$data['rubros'] = $this->Cliente->getRubros();
	...
	public function reenviar()	{
		if ( ! empty($_POST) ) {
			$datos = $_POST;
			$verificar = $this->Cliente->getClienteByCorreo($datos['correo']);
			if ($verificar) {
				if ($verificar['estado'] == _ACTIVO) {
					$this->session->set_flashdata('message', '<div class="alert alert-success">El correo ingresado se encuentra activo.</div>');
				} else {
					$datos['correo'] = $verificar['correo'];
					$datos['codigo'] = md5(time());
					$update = $this->Cliente->updateCliente($verificar['id'],$datos);
					if ($update) {
						...
						$mail->AddAddress($datos['correo']);#correo de usuario

						$mail->Subject = "Activación de cuenta :: ".dataConfig('nombre_comercial');
						$dato['data'] = $datos;
						$msg = $this->load->view('frontend/mailing/cliente_reenvio_activacion', $dato, true);
						$mail->Body = $msg;
						$mail->IsHTML(true);
						
						@$mail->Send();

						$this->session->set_flashdata('message', '<div class="alert alert-success">Se ha reenviado correctamente, Usted recibirá un correo para confirmar la activación de su cuenta.</div>');
						redirect(base_url().'clientes/login#redirectLogin');
					} else {
						$this->session->set_flashdata('message', '<div class="alert alert-warning">No existe el correo ingresado.</div>');
					}
			...
			redirect(base_url().'clientes/reenviar-activacion#redirectLogin');
			
		}

		$data = array();
		$data['body'] = 'reenviar-activacion';
        $this->parser->parse("frontend/includes/template", $data);
	}
	...
	public function nuevo() {
		...
		$names = array('correo|valid_email|is_unique[clientes.correo]','password','razon_social','direccion','ruc');
		if ( ! empty($_POST) ) {
			...
			if (validacionForm($names, 'controller') == FALSE){
				...
				# if($response['success']){
					...
					$datos['direccion_entrega'] = (isset($datos['ck_direccion']) && ($datos['ck_direccion'] == 'on'))? $datos['direccion'] : $datos['direccion_entrega'] ;
					unset($datos['g-recaptcha-response']);
					unset($datos['cfmPassword']);
					unset($datos['ck_direccion']);
					$datos['idioma_id'] = 1;
					$datos['estado_id'] = 1;
		...
		#captcha
		//$data['recaptcha'] = $this->recaptcha->render();
		$seo = array('seo_title'=>'Registrate :: '.dataConfig('nombre_comercial'),'seo_description'=>'Registrate.');
		$data['seo'] = $seo;
		$data['body'] = 'login';
        $this->parser->parse("frontend/includes/template", $data);
	}
	...
	public function update() {
		...
		$names = array('razon_social','telefono','direccion');
		if (! empty($_POST)) {
			if (validacionForm($names, 'controller') == FALSE){
				$this->form_validation->set_error_delimiters('<label class="error" style="color:#FF0000;">', '</label>');
			}else{
