    ...
    function getColorCotizacion($id_producto,$atributo,$idCarro){
        ...
        if ($items) {
            ...
            foreach ($items[1]['atributos'] as $key => $item) {
                ...
                $html .=        '<span class="colores color'.$idCarro.' '.$active.'" onclick="colorActiveBtn(\''.$item['id'].'\',\''.$idCarro.'\')" id="rdb'.$item['id'].'" title="'.$item['atributos_nombres'].'" style="background: '.$item['atributos_valor'].'"></span>';
                $html .=    '</label>';
                $i++;
                $required = '';
            }
...
if ( ! function_exists('calculoCotizacion')){
    function calculoCotizacion($id_producto, $cant_color, $id_impresion, $cantidad){
        ...
        $costoTotal = $pub*$cantidad+$costoDemasia+$costoMovilidad+$costoEmbalaje+$costoRegalo+$costoImpresion;
