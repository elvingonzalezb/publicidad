...
function actualizarEstado(param_id,param_estado,url){
    ...
    $.ajax({
        ...
        type:  'post',
        beforeSend: function () {
            $('#btn-'+param_id).text('Procesando...');
        },
        success:  function (response) {
            if (response.estado == 1){
                $('#btn-'+param_id).addClass('btn-success').removeClass('btn-warning');
                $('#btn-'+param_id).text('ACTIVO');
            } else {
                $('#btn-'+param_id).addClass('btn-warning').removeClass('btn-success');
                $('#btn-'+param_id).text('INACTIVO');
            }
            $('#estado'+param_id).val(response.estado);                             
        }
    });
}

function actualizarCotizacion(param_id){
    $.ajax({
        data:  {"param_id" : param_id},
        url:   'dashboard/cotizaciones/ajaxEstado',
        dataType: "json",
        type:  'post',
        beforeSend: function () {
            $('#idCot'+param_id).text('Procesando...');
        },
        success:  function (response) {
            $('#idCot'+param_id).html('<span style="font-size:13px;" class="label label-success">Aceptado</span>');
            html = '<a class="btn btn-xs btn-info" href="dashboard/cotizaciones/detalle/'+param_id+'"><i class="fa fa-eye icon-white"></i> VER</a> ';
            html += ' <a class="btn btn-xs btn-danger" href="javascript:borrarReserva('+param_id+')"><i class="fa fa-trash icon-white"></i> &nbsp;&nbsp;BORRAR</a>';
            $('#accion'+param_id).html(html);
        }
    });
}

function enviarCotiDolar(cotizacion_id) {
    swal({
        title: "¿Estás seguro?",
        text: "Enviar cotización en dolares(USD).",
        type: "warning",
        ...
        $.ajax({
            ...
            url:   'dashboard/cotizaciones/enviarCotiDolar',
            dataType: "json",
            type:  'post',
            success: function (response) {
                swal(response.titulo, response.msj, response.estado);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                swal("¡Error al enviar!", "Inténtalo de nuevo", "error");
            }
        });
    });
}

function eliminarItem(param_id,url) {
    swal({
        title: "¿Estás seguro?",
...
function calSubtotal(){
    ...
    rec_adelanto = $("#rec_adelanto").val(); 
    rec_adelanto = parseFloat(rec_adelanto);
    total = 0;
    total2 = 0;
    for(i=0; i<num_items; i++)
    {
        precio = $("#precio_"+i).val();
        precio2 =  parseFloat(precio).toFixed(2)*(100+rec_adelanto)/100;
        $("#precio5050_"+i).html('S/. '+ number_format(precio2));
        cantidad = $("#cantidad_"+i).val();
        aux_p_subtotal = parseFloat(precio*cantidad).toFixed(2);
        aux_p_subtotal2 = parseFloat(precio2*cantidad).toFixed(2);

        $("#id_subtotal_"+i).html('S/. '+ number_format(aux_p_subtotal));
        total = total + parseFloat(precio*cantidad);
        $("#id_subtotal5050_"+i).html('S/. '+ number_format(aux_p_subtotal2));
        total2 = total2 + parseFloat(precio2*cantidad);
    }
    aux_subtotal = parseFloat(total).toFixed(2);
    aux_subtotal2 = parseFloat(total2).toFixed(2);

    aux_igv = (aux_subtotal*18)/100;
    aux_igv2 = (aux_subtotal2*18)/100;
    
    aux_total = aux_subtotal*1.18;
    aux_total2 = aux_subtotal2*1.18;

    $("#celdaSubTotal").html('S/. '+ number_format(aux_subtotal));
    $('#input_subtotal').val(aux_subtotal);
    $("#celdaSubTotal5050").html('S/. '+ number_format(aux_subtotal2));
    $('#input_Subtotal5050').val(aux_subtotal2);

    $("#celdaIgv").html('S/. '+ number_format(aux_igv));
    $('#input_igv').val(aux_igv);
    $("#celdaIgv5050").html('S/. '+ number_format(aux_igv2));
    $('#input_igv5050').val(aux_igv2);

    $("#celdaTotal").html('S/. '+ number_format(aux_total.toFixed(2)));
    $('#input_total').val(aux_total);
    $("#celdaTotal5050").html('S/. '+ number_format(aux_total2.toFixed(2)));
    $('#input_total5050').val(aux_total2);
}

/**actualiza destacado producto**/
function actualizarDestacado(param_id,param_estado){
    if (param_estado == 1) { estado = 2;}else{estado = 1;}
    $.ajax({
        data:  {"param_id" : param_id,"param_estado" : estado },
        url:   'dashboard/productos/ajaxDestacado',
        dataType: "json",
        type:  'post',
        beforeSend: function () {
